...
arm_eliminator <- function(k, t, S_list, var_hats, alpha, epsilon){
  good_or_bad <- rep(0, length(S_list))
  for (i in 1:length(S_list)){
    S <- S_list[[i]]
    var_hat <- var_hats[[i]]
    bounds <- conf_int_hedged(S, var_hat, alpha/4, npts = 100)
    low <- bounds[1]
    upp <- bounds[2]
    # check if a good arm
    if (low > epsilon){
...
for (sim in 1:n_sims){
  ...
  while( length(active_set) != 0 ){
    ...
    t <- t+1
    var_hats[[which_arm]] <- c(var_hats[[which_arm]], 
                               (1/4 + sum( (S_list[[which_arm]]-mean(S_list[[which_arm]]))^2 ) )/(length(S_list[[which_arm]])+1))
    ...
    good_or_bad <- arm_eliminator(k, t, S_list, var_hats,  alpha, epsilon)
