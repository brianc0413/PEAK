...
# capital process value for projected minimum
e_value <- function(S, mu_hat, m){
  # calculate lambda_values
  lambdas <- (mu_hat - m)/(0.26)
  return(prod(1 + lambdas*(S - m)))
}
...
arm_eliminator <- function(k, t, S_list, mu_hats, alpha, epsilon){
  good_or_bad <- rep(0, length(S_list))
  
  # optimal values for each arm
  opt <- rep(0,4)
  e_vals <- rep(0,4)
  temps <- rep(0,4)
  
  opt[1] <- findZeros(sum( (2*x - mu_hats[[1]] - S_list[[1]] )/(0.26+ (S_list[[1]]-x)*(mu_hats[[1]]-x) ) ) ~ x,xlim=c(0,1))[1,1]
  opt[2] <- findZeros(sum( (2*x - mu_hats[[2]] - S_list[[2]] )/(0.26+ (S_list[[2]]-x)*(mu_hats[[2]]-x) ) ) ~ x,xlim=c(0,1))[1,1]
  opt[3] <- findZeros(sum( (2*x - mu_hats[[3]] - S_list[[3]] )/(0.26+ (S_list[[3]]-x)*(mu_hats[[3]]-x) ) ) ~ x,xlim=c(0,1))[1,1]
  opt[4] <- findZeros(sum( (2*x - mu_hats[[4]] - S_list[[4]] )/(0.26+ (S_list[[4]]-x)*(mu_hats[[4]]-x) ) ) ~ x,xlim=c(0,1))[1,1]
  
  for (j in 1:4){
    e_vals[j] <- 1/4 * e_value(S_list[[j]], mu_hats[[j]], opt[j])
    temps[j] <- 1/4 * e_value(S_list[[j]], mu_hats[[j]], epsilon)
  }
  
  for (i in 1:length(S_list)){
    value <- sum(e_vals[1:4 !=i]) + temps[i]
    # check if a good arm
    if (value > 1/alpha){
      if (opt[i] > epsilon){
        good_or_bad[i] <- 1
...
for (sim in 1:n_sims){
  ...
  while( length(active_set) != 0 ){
    ...
    which_arm <- active_set[sample_function(k, t, S_list, alpha, epsilon, method, active_set)]
    #which_arm <- sample(1:4, size=1, prob = rep(1/4, 4))
    t <- t+1
    mu_hats[[which_arm]] <- c(mu_hats[[which_arm]], 
                              (1/2 + sum( S_list[[which_arm]]) ) /(length(S_list[[which_arm]])+1))
    ...
    good_or_bad <- arm_eliminator(k, t, S_list, mu_hats,  alpha, epsilon)
