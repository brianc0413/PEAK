...
upp_bounds <- rep(1, 5000)
low_bounds <- rep(0, 5000)
...
avg_peek <- sum_peek_conf_int/30
avg_bernoulli <- sum_bernoulli_conf_int/30
avg_bernstein<- sum_bernstein_conf_int/30
avg_hedged <- sum_hedged_conf_int/30
avg_prplh <- sum_prplh_conf_int/30

plot(x = 0:5000, y = avg_peek$lower, type = "l", lwd=2, col = "blue", ylim= c(0.2, 0.8))+
...
plot(x = 0:5000, y = log(avg_peek$upper -avg_peek$lower), type = "l", lwd=2, col = "blue", ylim = c(-3, -1.5), xlab = "t", ylab = "Log Volume of Confidence Sequence", main = "(D):Beta(0.25, 1)")+
